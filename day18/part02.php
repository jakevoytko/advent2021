...
$max_pop_pop = PHP_INT_MIN;

for ($i = 0; $i < count($all_numbers); $i++) {
  for ($j = 0; $j < count($all_numbers); $j++) {
    if ($i === $j) {
      continue;
    }
    $max_pop_pop = max($max_pop_pop, magnitude(addNumbers($all_numbers[$i], $all_numbers[$j])));
  }
}

print("The max magnitude is {$max_pop_pop}\n");
