...
function countPaths(array $neighbor_labels, array $uppercase_map, array &$seen_count, string $current_label, int $smol_caves_double_visited): int {
  ...
  foreach ($neighbor_labels[$current_label] as $neighbor_label) {
    // Big caves can always be iterated to.
    if ($uppercase_map[$neighbor_label]) {
      $sum += countPaths($neighbor_labels, $uppercase_map, $seen_count, $neighbor_label, $smol_caves_double_visited);
    } else {
      // The smol cave hasn't been visited at all.
      if (($seen_count[$neighbor_label] ?? 0) === 0) {
        $sum += countPaths($neighbor_labels, $uppercase_map, $seen_count, $neighbor_label, $smol_caves_double_visited);
      } else if ($smol_caves_double_visited === 0 && $neighbor_label !== 'start' && $neighbor_label !== 'end') {
        // This is the only double-visit allowed.
        $sum += countPaths($neighbor_labels, $uppercase_map, $seen_count, $neighbor_label, $smol_caves_double_visited + 1);
      }
    }
  }
...
$seen_counts = [];
$final_count = countPaths($neighbor_map, $uppercase_map, $seen_counts, 'start', 0);
