...
foreach ($bingo_calls as $current_number) {
  $cached_board = null;
  $bingo_boards = array_filter($bingo_boards, function(BingoCard $card) use ($current_number, &$sum_of_unmarked_numbers): bool {
    if ($card->callNumber($current_number)) {
      $sum_of_unmarked_numbers = $card->sumOfRemainingScores();
      return false;
    }
    return true;
  });

  if (empty($bingo_boards)) {
